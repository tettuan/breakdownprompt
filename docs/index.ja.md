...
ライブラリの基本的な使用方法、インストール方法、クイックスタートガイドなど、開発者が最初に知るべき情報を提供します。

| 資料名                                | 説明                                                                                                                   | 参照タイミング                   |
| ------------------------------------- | ---------------------------------------------------------------------------------------------------------------------- | -------------------------------- |
| [ユーザーガイド](./user_guide.md)     | ライブラリの基本的な使用方法、インストール手順、クイックスタートガイドを提供。実際の使用例とベストプラクティスを含む。 | ライブラリの使い方が分からない時 |
| [APIリファレンス](./api_reference.md) | 公開APIの詳細な仕様、インターフェース、型定義、エラーハンドリングの説明。各メソッドの使用方法と例を含む。              | APIの詳細な仕様を確認したい時    |

## 設計・仕様

システムの設計思想、アーキテクチャ、コンポーネント間の関係性など、システムの構造に関する情報を提供します。

| 資料名                                    | 説明                                                                                                             | 参照タイミング               |
| ----------------------------------------- | ---------------------------------------------------------------------------------------------------------------- | ---------------------------- |
| [設計パターン](./design_pattern.ja.md)    | システムの設計思想、アーキテクチャ、コンポーネントの責務と関係性を説明。各コンポーネントの詳細な振る舞いを含む。 | システムの構造を理解したい時 |
| [変数定義](./variables.ja.md)             | 変数の概念、予約変数とテンプレート変数の関係性、変数の検出と置換処理の仕様を説明。                               | 変数の扱い方に迷った時       |
| [変数の型定義](./type_of_variables.ja.md) | 変数の型システムの実装詳細、バリデーションルール、型の拡張方法を説明。TypeScriptの型定義の詳細を含む。           | 変数の型定義を確認したい時   |
| [返却仕様](./return_specification.ja.md)  | APIの返却値の仕様、エラーハンドリング、結果の分類と処理方法を説明。                                              | 戻り値の形式を確認したい時   |

## 開発ガイド

開発時のルール、規約、ベストプラクティスなど、開発者が従うべき指針を提供します。

| 資料名                                 | 説明                                                                                                     | 参照タイミング             |
| -------------------------------------- | -------------------------------------------------------------------------------------------------------- | -------------------------- |
| [Import Policy](./import_policy.ja.md) | モジュールのインポート規則、バージョン管理、セキュリティ考慮事項を説明。Denoのベストプラクティスを含む。 | 新しい依存関係を追加する時 |
| [パス検証ルール](./path_validation.md) | ファイルパスとディレクトリパスの検証ルール、セキュリティ考慮事項、エラーメッセージを説明。               | パス処理を実装する時       |

## テスト・品質

テスト戦略、品質管理、デバッグ方法など、品質を確保するための情報を提供します。

| 資料名                             | 説明                                                                                     | 参照タイミング           |
| ---------------------------------- | ---------------------------------------------------------------------------------------- | ------------------------ |
| [テスト](./test.ja.md)             | テスト階層構造、実行方法、デバッグモードの使用方法を説明。テストデータの管理方法を含む。 | テストを書く時           |
| [特殊文字の扱い](./priority.ja.md) | 特殊文字の処理ルール、セキュリティ考慮事項、テスト要件を説明。                           | 特殊文字の処理に迷った時 |

## セキュリティ・運用

セキュリティ対策、運用時の注意点、トラブルシューティングなど、運用に関する情報を提供します。

| 資料名          | 説明                                                                                         | 参照タイミング                 |
| --------------- | -------------------------------------------------------------------------------------------- | ------------------------------ |
| [UML図](./uml/) | システムの構造、コンポーネント間の関係性、処理フローを視覚的に表現。各種ダイアグラムを含む。 | システムの全体像を把握したい時 |
...
変数は以下の関係性で構成される。詳細な関係性については、[変数の関係性の詳細](./variables.ja.md#変数の関係性)を参照してください。

1. 予約変数
   - 事前に定義された変数名と対応するクラス
   - 変数名は固定で、対応するクラスが存在する
   - 例：`schema_file`, `input_text`, `input_text_file`, `destination_path`
...
3. テンプレート変数
   - テンプレート内の `{variable_name}` 記法で抽出される
   - 変数名はテンプレートの記載に依存
   - 予約変数と一致した場合のみ置換処理される

4. 変数置換処理
   - テンプレート変数が予約変数と一致した場合
   - 対応する予約変数のクラスが処理を実行
   - variables の値が存在する場合は、その値を使用
   - 値が存在しない場合は、クラスの定義に従って処理
...
詳細なユースケースについては、[ユーザーガイド](./user_guide.ja.md)を参照してください。
APIの詳細な仕様については、[APIリファレンス](./api_reference.ja.md)を参照してください。
...
詳細は[テスト方針](./test.ja.md)を参照してください。
