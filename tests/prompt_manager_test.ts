...
Deno.test("PromptManager - variable replacement accuracy", async () => {
  ...
  assert(
    content.includes(`${TEST_CONFIG.BASE_DIR}/schema/${TEST_PARAMS.LAYER_TYPE}.json`),
    "Schema file should be replaced with correct path",
  );
  assert(
    content.includes(`${TEST_CONFIG.BASE_DIR}/input/${TEST_PARAMS.FROM_LAYER_TYPE}.md`),
    "Input file should be replaced with correct path",
  );
  assert(
    content.includes(TEST_CONFIG.OUTPUT_DIR),
    "Destination should be replaced with correct path",
  );
  ...
  for (const [varName, originalValue] of variables) {
    ...
    if (expectedValue) {
      assert(
        content.includes(String(expectedValue)),
        `Variable ${varName} should be replaced with correct value`,
      );
    }
  }
